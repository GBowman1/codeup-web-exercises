...
function onDragEnd() {
    ...
    fetch("https://api.openweathermap.org/data/2.5/forecast?lat=" + lngLat.lat + "&lon=" + lngLat.lng + `&appid=${WM_KEY}&units=${units}`)
        .then(res => res.json())
        .then(data => {
            let children = document.querySelector("#weather-data").children
            for (let i = 0; i < data.list.length; i += 8) {
            ...
            // change location text in top right of page
            document.querySelector("#current-city").innerText = `Current City: ${data.city.name}, ${data.city.country}`
        })
}
